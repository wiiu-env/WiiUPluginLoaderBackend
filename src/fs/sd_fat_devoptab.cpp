...
#define mymalloc(x) ((void * (*)(size_t))(*pMEMAllocFromDefaultHeap))(x)
#define myfree(x) ((void (*)(void *))(*pMEMFreeToDefaultHeap))(x)
...
static int sd_fat_dirnext_r (struct _reent *r, DIR_ITER *dirState, char *filename, struct stat *st) {
    ...
    FSDirEntry * dir_entry = (FSDirEntry *)mymalloc(sizeof(FSDirEntry));
...
static int sd_fat_add_device (const char *name, const char *mount_path, void *pClient, void *pCmd) {
    ...
    priv->pCmd = pCmd;
    priv->pMutex = mymalloc(OS_MUTEX_SIZE);
...
int32_t mount_sd_fat(const char *path) {
    ...
    void* pClient = mymalloc(FS_CLIENT_SIZE);
    void* pCmd = mymalloc(FS_CMD_BLOCK_SIZE);
